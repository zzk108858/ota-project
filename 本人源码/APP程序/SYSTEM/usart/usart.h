...
#define USART3_RX_SIZE				1024*5	//定义接收缓冲区总大小
#define USART3_TX_SIZE				1024*5	//定义发送缓冲区总大小
#define USART3_REC_LEN  			512  	//定义单次最大接收字节数 256
...
typedef struct{
	u16 UTxCount;										//记录发送缓冲区内数据量
	u16 URxCount;										//记录接收缓冲区内数据量
	u16 UTxState;                  //0:发送空闲  1：忙碌
	...
	UCB_URxBufptr *UTxDataIN;				//写位置
	UCB_URxBufptr *UTxDataOUT;			//读位置
	UCB_URxBufptr *UTxDataEND;			//最后一段缓冲区的地址，用来判断是否需要回头
